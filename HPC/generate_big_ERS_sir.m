function cmds = generate_big_ERS_sir(ERS_num,year,day,last_day)
...
addpath(genpath("/home/alexmc99/ERS_work/scripts"));
...
for dstart=day:6:last_day
    ...
    % for interval=[0] % test for one day
      ...
      for ireg=1:3
        RegCh=Reg(ireg);
        % create make file
        cmd=sprintf('/home/dgl2/taskman/ERS/bin/ers_meta_make -r /home/alexmc99/ERS_work/scripts/ERS/ref -M %s/E2%c_%4.4d_%3.3d_%2.2d.meta ESCAT %d %d %d /home/alexmc99/ERS_work/scripts/ERS/def/escat_E2%cB.def %s/%4.4d_%3.3d_%2.2d.flist',wdir,RegCh,year,dstart,interval,dstart,eday,year,RegCh,wdir,year,dstart,interval);
        disp(cmd);
        if ~dry_run
            system(cmd);
        end
        % create GRD and SIR product file
        cmd=sprintf('/home/dgl2/taskman/ERS/bin/ers_cetb_sir_big -x /home/alexmc99/ERS_work/scripts/ERS/bin/fixnc %s -R -s -w %s %s/E2%c_%4.4d_%3.3d_%2.2d.meta',pdir,wdir,wdir,RegCh,year,dstart,interval);
        disp(cmd);
	pmes_env="PMESDR_TOP_DIR=/home/dgl2/taskman"
	disp(pmes_env);
	system(pmes_env);
        tstart=tic;
        if ~dry_run
...
% /auto/home/long/src/linux/newSCP/ERS/bin/ers_cetb_sir_big -x /auto/home/long/src/linux/newSCP/TRMMPR/bin/fixnc /auto/temp/long/TRMMPR/final -R -s -w work work/E2T_1998_180_006.meta
