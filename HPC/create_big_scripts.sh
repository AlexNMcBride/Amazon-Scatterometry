...
log_file="/home/alexmc99/ERS_work/scripts/logs/big_generate_log.txt"
...
for year in $(seq $start_year $end_year); do
    ...
    for date in $(seq $first_date 6 $last_date); do
    	slurm_setup="
		#! /bin/bash\n#SBATCH --time=24:00:00\n#SBATCH --mem-per-cpu=49182M\n#SBATCH --ntasks=1\n#SBATCH --mail-user=alexmc99@byu.edu\n#SBATCH --mail-type=END\n#SBATCH --mail-type=FAIL\n\n#SBATCH -J "$year-$date-ERS$ers_num"\nexport OMP_NUM_THREADS=1\n\nmodule load matlab\n"

  	filename="$year"_"$date"_ERS"$ers_num".sh
    	echo -e $slurm_setup > $script_path/$filename
        cmd="matlab -nodisplay -nojvm -nosplash -r \"addpath(genpath(\\\"/home/alexmc99/ERS_work/scripts\\\")); generate_big_ERS_sir($ers_num, $year, $date, $date); exit\""
